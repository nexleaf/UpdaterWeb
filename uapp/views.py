...
def getClientInfo(request, imei):
    ...
    if len(user)>0:
        user=user[0]
    user_info = {}
    user_info['imei']=user_imei
    print user.group.all()[0].name
    user_info['group'] = user.group.all()[0].name
    user_info['phone'] = user.phone
    user_info['sim_id'] = user.simid
    return render_to_response('client_info.html',{'user_info':user_info})
...
def show_clients(request):
    ...
    # setattr(UserForm,"user_group", forms.ChoiceField(label="group",choices = CHOICES,))
...
def group_details(request, groupname):
    ...
    return render_to_response('group_info.html',{"group": group,"apps":apps})
def app_details(request):
    ...
    # result['descr']=group.descr
    # return HttpResponse(result_json, content_type='application/javascript')
    return render_to_response('app_info.html',{"app":app})
...
def client_details(request, imei):
    ...
    # result['descr']=group.descr
    # return HttpResponse(result_json, content_type='application/javascript')
    return render_to_response('client_info.html',{"user": user,"apps":apps,"group_name":group_name})
...
def edit_client(request):
    ...
    if len(user_q) > 0:
        ...
        group = Group.objects.filter(name=request.POST.get("group_name"))
        user.group = group
...
def getReleaseVersions(request, appname, release):
    ...
    return render_to_response('show_application_versions.html',{"versionlist": versionlist,"app_name":appname,"release":release})
